      ...
      if(hasNonZeroElement(circlenum) || hasNonZeroElement(ptjobnum)) {
        ...
        async function select7() {
          ...
          if(flg1){
            ...
            sendDialogmsg(document.getElementById('msg').innerHTML);
          }
...
async function selectCourse(type, eventNum) {
  return new Promise(function(resolve) {
    ...
    var specialty = parseInt(sessionStorage.getItem('specialty'));
    var sociability = parseInt(sessionStorage.getItem('sociability'));
    var major = parseInt(sessionStorage.getItem('major'));
    var circle = JSON.parse(sessionStorage.getItem('circle'));
    ...
    if(major === 3){  //理工
        ...
        if(sociability > 300*0.8 && specialty > 60*0.8){
        ...
        if(sociability > 330*0.8 || specialty > 40*0.8){
              ...
              if (buttonId === "applyButton") // 行く
              {
                ...
                async function select6() {
                  ...
                  } else if (buttonId === "country4") 
                  {
                    div2.remove();
                    // ダイアログ表示
                    document.getElementById('msg').textContent = abroadjson.abroad[3].country + eventjson[type][eventNum].msg.msg2;
                    sendDialogmsg(document.getElementById('msg').innerHTML);
                    // ステータス変化の反映および表示
                    var money = eventjson[type][eventNum].money;
                    var sociability = eventjson[type][eventNum].sociability;
...
async function afterGoal() {
  return new Promise(function(resolve) {
    ...
    .then(async data => {
      // "name"が一致するオブジェクトを探す
      const nameToFind = sessionStorage.getItem('course'); // 参照したいオブジェクトの"name"
      const matchingObject = data.course.find(item => item.name === nameToFind);

      if (matchingObject) {
        // オブジェクトが見つかった場合、"gexp"を参照する
        const gexpValue = matchingObject.gexp;
        // セッションストレージに格納
        await setSessionStorage(0, 0, 0, 0, gexpValue);
        // ステータス変化を表示
        var msg = await makeStatusmsg(0, 0, 0, 0, gexpValue);
        ...
        console.log("該当するオブジェクトが見つかりませんでした。");
      }
    })
...
async function gameStart() {
  ...
  // 他のプレイヤーのアイコンも表示
  socket.emit('playerList2');
  socket.on('playerList2', playerList => {
    for(let i = 0; i < playerList.length; i++)
    {
      if(playerList[i] !== sessionStorage.getItem('name'))
      {
        var icon = document.createElement("img");
        icon.classList.add("playicon");
        icon.id = playerList[i];
        icon.src = "icon" + sessionStorage.getItem(playerList[i]) + ".png";
        icon.alt = "icon";
        document.getElementById("event1-0-1").appendChild(icon);
      }
    }
...
async function game(){
  ...
  {
    // ゴールしている場合
    document.getElementById('msg').textContent = "既にゴールしているので経験値を手に入れた！";
    sendDialogmsg(document.getElementById('msg').innerHTML);
    // ゴール後の処理
    await afterGoal();
  }
  else
  {
    ...
    if(location === 50 || 81 || 112 || 157){
      switch(location){
      case 50:
        setActiveButton(1);
        await toMap2();
        await moveImage(location, location + 1);
      ...
      case 157:
        ...
        socket.emit('home', [sessionStorage.getItem('map'), sessionStorage.getItem('exgrid')])
        break;
      }
    }
    ...
    else
    {
      ...
      sendDice(dice); // サーバーにサイコロの目を送信

      // サイコロの目に応じてで移動
      now = document.getElementById('playicon').parentNode.className;
      array = now.split(" ");
